    ...
    <div v-else class="vstack gap-3">
      <div v-for="g in orderGroups" :key="g.groupKey" class="card shadow-sm rounded-4">
          ...
          >
            ...
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-2">
              <div class="small">
                ...
                <span v-if="g.tierIsFreeDelivery" class="badge text-bg-success-subtle border ms-2"
                  >Free-delivery perk</span
                >
              ...
              <div class="small">
                <template v-if="isFreeShippingCandidate(g)">
                  <span class="badge text-bg-success-subtle border"
                    >Candidate for free shipping</span
                  >
                </template>
                <template v-else>
                ...
                <template v-if="groupFreeShipping(g.reference_number)">
                  <div class="small text-muted" v-if="shippingForRef(g.reference_number) > 0">
                    ...
                    <span class="text-decoration-line-through"
                      >₱ {{ number(shippingForRef(g.reference_number)) }}</span
                    >
                  </div>
        ...
        <div v-if="g.containsRR" class="mt-3">
          ...
          <div v-for="rr in groupRRs(g)" :key="rr.id" class="border rounded p-2 bg-body small mb-2">
            ...
            <div class="d-flex align-items-center gap-3">
              ...
              <div class="flex-grow-1">
                ...
                >
                  ...
                  <span class="small">
                    <template
                      v-if="!hasEventDiscount(g.reference_number) && !hasItemOnlyDiscount(s)"
                    >
          ...
          >
            ...
            <div class="flex-grow-1">
              ...
              <div
                class="small"
                :class="
                  hasEventDiscount(g.reference_number) || hasItemOnlyDiscount(s) ? '' : 'text-muted'
                "
              >
                <template v-if="!hasEventDiscount(g.reference_number) && !hasItemOnlyDiscount(s)">
                  ₱ {{ number(s.line_total) }}
                ...
                <template v-else>
                  ...
                  <span class="fw-semibold">₱ {{ number(lineTotalAfterItemOnly(s)) }}</span>
                </template>
              </div>
              ...
              <div
                class="small mt-1"
                v-if="
                  purchaseStatusKey(g, s.order_id) === STATUS.TO_RECEIVE &&
                  purchaseTrackingLink(g, s.order_id) &&
                  !refTrackingLink(g.reference_number)
                "
              >
      ...
      <div class="orders-modal-body">
        ...
        <div class="row g-3">
          <div class="col-12 col-lg-6">
            ...
            <div class="border rounded p-2 bg-body small">
              <div class="d-flex align-items-center gap-2 mb-1">
                ...
                >
                  <img
                    v-if="selectedGroup && buyerAvatar(selectedGroup.purchases[0]?.user_id)"
                    :src="buyerAvatar(selectedGroup.purchases[0]?.user_id)!"
                    alt="Buyer avatar"
                    class="w-100 h-100 object-fit-cover"
                  />
                  <i v-else class="bi bi-person-circle text-muted fs-4"></i>
          ...
          <div class="col-12 col-lg-6">
            ...
            <div class="border rounded p-2 bg-body small">
              ...
              <div
                v-if="
                  hasEventDiscount(selectedGroup.reference_number) &&
                  groupItemDiscountAmount(selectedGroup) > 0
                "
              >
              ...
              <div
                v-if="
                  totalDiscountsCreditedForRef(selectedGroup.reference_number) > 0 &&
                  !hasEventDiscount(selectedGroup.reference_number)
                "
              >
              ...
              <div v-if="statusFilter !== STATUS.RETURN_REFUND" class="mt-1">
                <template v-if="groupFreeShipping(selectedGroup.reference_number)">
                  ...
                  >
                    Shipping fee:
                    <span class="text-decoration-line-through"
                      >₱ {{ number(shippingForRef(selectedGroup.reference_number)) }}</span
                    >
                  </div>
                  <div class="small text-success fw-semibold">Free shipping</div>
                </template>
                <div
                  class="small text-muted"
                  v-else-if="shippingForRef(selectedGroup.reference_number) > 0"
                >
              ...
              <div v-if="selectedGroupHasToShip" class="mt-2">
                ...
                <div class="form-text">
                  One link for the whole reference. Required before shipping.
                </div>
              </div>
            </div>
        ...
        <div class="mt-3">
              ...
              <div class="flex-grow-1">
                ...
                <div class="mt-1">
                  ...
                  <template v-else-if="hasEventDiscount(selectedGroup.reference_number)">
                    <span class="text-muted small">Line total:</span>
                    <span
                      class="text-muted small text-decoration-line-through me-1"
                      v-if="
                        number(o.items[0]?.line_total) !==
          ...
          <div class="d-flex flex-column align-items-end mt-3">
            ...
            <div
              class="small text-muted"
              v-if="itemOnlySumForRef(selectedGroup.reference_number) > 0"
            >
              Item-only discounts: − ₱
              {{ number(itemOnlySumForRef(selectedGroup.reference_number)) }}
            </div>
            ...
            <div
              class="small text-muted"
              v-if="orderWideSumForRef(selectedGroup.reference_number) > 0"
            >
...
function swalError(message: string, title = 'Error') {
  return Swal.fire({
    icon: 'error',
    title,
    text: message,
...
function swalSuccess(message: string, title = 'Success') {
  return Swal.fire({
    icon: 'success',
    title,
    text: message,
...
function swalInfo(message: string, title = 'Info') {
  return Swal.fire({
    icon: 'info',
    title,
    text: message,
...
const avatarUrlMap: Record<string, string> = reactive({})
const avatarSigningBusy: Record<string, boolean> = reactive({})
...
/* === NEW: helpers for reference-based messages (whole reference, not per item) === */
function refFromPurchaseId(purchaseId: string): string | null {
  const direct = purchaseRefIndex[purchaseId]
  if (direct) return direct
  const o = orders.value.find((oo) => oo.id === purchaseId)
  return o?.reference_number || null
}
function refLabel(ref?: string | null): string {
  return ref ? `Reference #${ref}` : 'this reference'
}
...
function buyerAvatar(userId?: string | null): string | null {
  if (!userId) return null
  const buyer = buyersMap[userId]
  const raw = buyer?.profile_url || ''
  if (!raw) return null
  ...
  // Cached signed URL?
  if (avatarUrlMap[userId]) return avatarUrlMap[userId]

  // Create signed URL from user_profile bucket
  if (!avatarSigningBusy[userId]) {
    avatarSigningBusy[userId] = true
    supabase.storage
      .from('user_profile')
      .createSignedUrl(raw, 3600)
      .then(({ data, error }) => {
        if (!error && data?.signedUrl) {
          avatarUrlMap[userId] = data.signedUrl
        }
      })
      .finally(() => {
        avatarSigningBusy[userId] = false
      })
  }
...
const orderGroups = computed<ViewGroup[]>(() => {
  ...
  for (const [ref, arr] of Object.entries(byRef)) {
    ...
    const netAfterDiscounts = Math.max(
      0,
      Number((itemsTotal - discount_total_effective).toFixed(2)),
    )
  ...
  if (statusFilter.value === STATUS.COMPLETED) {
    out = out.filter((g) => {
      const allForRef: any[] = (siblingsByRef[g.reference_number] || []).concat(
        g.purchases.map(
          (p) =>
            ({
              id: p.id,
...
function isGroupPendingShipping(g: ViewGroup): boolean {
  const feePending =
    !groupFreeShipping(g.reference_number) && shippingForRef(g.reference_number) <= 0
  const allPending = g.purchases.length > 0 && g.purchases.every((p) => isPending(p.status))
...
async function loadOrders(resetPage = false) {
  ...
  try {
    ...
    if (statusFilter.value !== 'all' && statusFilter.value !== TAB_OFFER_SHIPPING)
    ...
    if (error) {
      orders.value = []
      total.value = 0
      await swalError(error.message || 'Failed to load orders.', 'Load Error')
      return
    }
    ...
    if (userIds.size > 0) {
      const { data: urows } = await supabase
        .from('users')
        .select('id, full_name, phone_number, address, membership_id, profile_url')
        ...
        for (const t of trows as TierRow[]) {
          tiersMap[t.id] = {
            ...
            purchase_requirements_for_free_delivery:
              Number(t.purchase_requirements_for_free_delivery || 0) || 0,
          }
        }
      }
    }
    ...
    if (needRR) {
      let rrQ = supabase
        ...
        .from('return_refunds')
        .select(
          'id,user_id,purchase_id,product_id,reason,details,status,created_at,updated_at,refund_tracking_link',
        )
    ...
    if (purchaseIds.size > 0) {
      ...
      const discountIds = Array.from(
        new Set(allRedemptions.map((d) => d.discount_id).filter(Boolean)),
      )

      // lookup discounts metadata
      const discountMeta: Record<
        string,
        {
          id: string
          title: string | null
...
/* ---------- Actions (updated to use SweetAlert but keep logic) ---------- */
async function updateStatus(purchaseId: string, status: string): Promise<boolean> {
  ...
  try {
    ...
    if (error) {
      await swalError(error.message || 'Failed to update status.', 'Update Error')
      return false
    }
...
async function markAsShipped(purchaseId: string, silent = false) {
  const ref =
    purchaseRefIndex[purchaseId] || orders.value.find((o) => o.id === purchaseId)?.reference_number
  const link = ref && trackLinkByRef[ref] ? trackLinkByRef[ref].trim() : ''
  if (!link) {
    await swalError(
      'Please enter the reference tracking link before marking as shipped.',
  ...
  try {
    ...
    if (error) {
      await swalError(error.message || 'Failed to update order status.', 'Update Error')
      return
    }
...
async function applyTxPesos(
  ...
  if (error) {
    ...
    await swalError(error.message || 'E-wallet transaction failed.', 'E-Wallet Error')
    return null
  }
...
async function cancelOrder(purchaseId: string, skipConfirm = false, silent = false) {
  const order = orders.value.find((o) => o.id === purchaseId)
  const refForMsg = order?.reference_number || refFromPurchaseId(purchaseId)

  if (!skipConfirm) {
    const ok = await swalConfirm({
      title: `Cancel order for ${refLabel(refForMsg)}?`,
  ...
  try {
    if (shouldRefundEwallet) {
      ...
      if (upErr) {
        await swalError(upErr.message || 'Failed to cancel order.', 'Cancel Error')
        return
      }
      ...
      if (!silent) {
        await swalSuccess(
          `Order under ${refLabel(refForMsg)} has been cancelled and refunded.`,
          'Order Cancelled',
        )
      }
      return
    ...
    if (shouldInsertCODReceipt) {
      ...
      if (!silent) {
        await swalSuccess(
          `COD order under ${refLabel(refForMsg)} has been cancelled.`,
          'Order Cancelled',
        )
      }
      return
    ...
    const ok = await updateStatus(purchaseId, STATUS.CANCELLED)
    await restoreStock(productIdForStock, qtyForStock)
...
/* Approve order (kept, now with optional group-silent) */
async function approveOrder(order: ViewOrder, silent = false) {
  const purchaseId = order.id
  busy.value.action[purchaseId] = true
  try {
    if (order.status !== STATUS.TO_PAY) {
      await swalError('Only "To Pay" orders can be approved.', 'Invalid Status')
      return
    }
    ...
    if (upErr) {
      await swalError(upErr.message || 'Failed to update order status.', 'Update Error')
      return
    }
...
/* Approve/Reject/Complete Refund (kept + SweetAlerts per reference) */
async function approveRefund(rr: ReturnRefundRow) {
  if (!rr) return
  const rrId = rr.id
  const link = (rrTrackById[rrId] || '').trim()
  if (!link) {
    await swalError('Please enter the return tracking link before approval.', 'Missing Link')
    return
  }
  ...
  try {
    const { error } = await supabase
      ...
      .eq('id', rrId)
    if (error) {
      await swalError(error.message || 'Failed to approve refund.', 'Update Error')
      return
    }
    rr.status = 'approved'
...
async function rejectRefund(rr: ReturnRefundRow) {
  ...
  const ok = await swalConfirm({
    ...
    text: 'The customer will not receive a refund.',
    icon: 'warning',
  ...
  try {
    const { error } = await supabase
      ...
      .eq('id', rrId)
    if (error) {
      await swalError(error.message || 'Failed to reject refund.', 'Update Error')
      return
    }
    rr.status = 'rejected'
    ...
    if (!pErr && purchase) {
      ...
      if (!prodErr && product) {
        ...
        let effectiveEach =
          discountedByPurchase[purchase.id] != null
        ...
        await swalSuccess(
          `Refund request for ${refLabel(purchase.reference_number)} has been rejected.`,
          'Refund Rejected',
        )
      }
    }
...
async function completeRefund(rr: ReturnRefundRow) {
  if (!rr) return
  if (!isRRApproved(rr)) {
    await swalError('Only "Approved" refunds can be completed.', 'Invalid Status')
    return
  }
  ...
  try {
    const { data: purchase, error: pErr } = await supabase
      ...
      .single()
    if (pErr || !purchase) {
      await swalError(pErr?.message || 'Purchase not found for this refund.', 'Not Found')
      return
    }
    ...
    const { data: product, error: prodErr } = await supabase
      ...
      .single()
    if (prodErr || !product) {
      await swalError(prodErr?.message || 'Product not found for this refund.', 'Not Found')
      return
    }
    ...
    if (!isFinite(productRefund) || productRefund <= 0) {
      await swalError('Invalid refund amount computed.', 'Invalid Amount')
      return
    }
    ...
    const { error: rrUpdateErr } = await supabase
      ...
      .single()
    if (rrUpdateErr) {
      await swalError(rrUpdateErr.message || 'Failed to update refund status.', 'Update Error')
      return
    }
    ...
    const refText = refLabel(purchase.reference_number || refFromPurchaseId(rr.purchase_id))
    await swalSuccess(
      `Refund for ${refText} has been completed and credited to the customer's e-wallet.`,
      'Refund Completed',
    )
  } finally {
    busy.value.action[rrId] = false
...
/* GROUP ACTIONS (kept, now with group-level SweetAlerts by reference) */
async function approveGroup(g: ViewGroup) {
  let changed = 0
  for (const o of g.purchases) {
    if (o.status === STATUS.TO_PAY) {
      await approveOrder(o, true /* silent per item */)
      changed++
    }
  }
  ...
  if (changed > 0) {
    await swalSuccess(
      `${changed} order(s) under ${refLabel(g.reference_number)} have been approved and moved to "To Ship".`,
      'Group Approved',
    )
  } else {
    await swalInfo(
      `No "To Pay" orders found under ${refLabel(g.reference_number)}.`,
      'Nothing to Approve',
    )
  }
}
...
async function markGroupAsShipped(g: ViewGroup) {
  if (!canGroupMarkAsShipped(g)) {
    await swalError(
      'Please provide the reference tracking link before marking as shipped.',
      'Missing Tracking Link',
    )
    return
  }
  ...
  for (const o of g.purchases) {
    if (o.status === STATUS.TO_SHIP) {
      await markAsShipped(o.id, true /* silent per item */)
      changed++
    }
  }
  ...
  if (changed > 0) {
    await swalSuccess(
      `${changed} order(s) under ${refLabel(g.reference_number)} have been marked as "To Receive".`,
      'Group Updated',
    )
  } else {
    await swalInfo(
      `No "To Ship" orders found under ${refLabel(g.reference_number)}.`,
      'Nothing to Update',
    )
  }
}
async function cancelGroup(g: ViewGroup) {
  const ok = await swalConfirm({
    title: `Cancel all purchases under ${refLabel(g.reference_number)}?`,
    text: 'This will cancel every order in this group.',
    icon: 'warning',
  ...
  let changed = 0
  for (const o of g.purchases) {
    await cancelOrder(o.id, true /* skipConfirm */, true /* silent */)
    changed++
  }

  if (changed > 0) {
    await swalSuccess(
      `All ${changed} order(s) under ${refLabel(g.reference_number)} have been cancelled.`,
      'Group Cancelled',
    )
  }
}
...
/* NEW: clear ?focus= from URL when closing the modal */
function clearFocusQueryFromUrl() {
...
function closeGroupModal() {
  ...
  // when closing from X/close button, remove ?focus=... from URL
  clearFocusQueryFromUrl()
}
...
/* ===== Auto-open modal from ?focus=REF query ===== */
function getFocusRefFromRoute(): string | null {
...
watch(
  [getFocusRefFromRoute, () => orderGroups.value],
  ([focusRef]) => {
    if (!focusRef) return
    const g = orderGroups.value.find((gg) => gg.reference_number === focusRef)
    if (!g) return
    if (selectedRef.value === focusRef) return
    openGroupModal(g)
  },
  { immediate: true },
)
...
/* ===== Offer Shipping actions (kept, with SweetAlert per reference) ===== */
async function offerShipping(g: ViewGroup) {
  const ref = g.reference_number
  const raw = getShippingInputRaw(ref)
  const amount = Number(raw)
  if (!isFinite(amount) || amount < 0) {
    await swalError('Please enter a valid shipping fee (₱).', 'Invalid Shipping Fee')
    return
  }
  ...
  try {
    ...
    if (error) {
      await swalError(error.message || 'Failed to save shipping fee.', 'Update Error')
      return
    }
...
async function makeFreeShipping(g: ViewGroup) {
  ...
  try {
    ...
    if (error) {
      await swalError(error.message || 'Failed to mark as free shipping.', 'Update Error')
      return
    }
...
/* Add this class to any section/div you want to "breathe in" on mount */
.breath-in-section {
  animation-name: orders-breath-in;
  animation-duration: 0.25s; /* 250ms as requested */
  animation-timing-function: ease-out;
  animation-fill-mode: both;
}

@keyframes orders-breath-in {
  0% {
    opacity: 0;
